...
#if 0
	dumpMemory8((PUINT_8)&(prStaRec->qosMapSet[0]), 64);
#endif
}
...
UINT_8 getUpFromDscp(IN P_GLUE_INFO_T prGlueInfo, IN int type, IN int dscp)
{
	...
	if ((type < 0) || (type > HW_BSSID_NUM)) {
		DBGLOG(INIT, WARN, "invalid network type %d.\n", type);
		return 0xff;
	}

	prAisBssInfo = prGlueInfo->prAdapter->aprBssInfo[type];
