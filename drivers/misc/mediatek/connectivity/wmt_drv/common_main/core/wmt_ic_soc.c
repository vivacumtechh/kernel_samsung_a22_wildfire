...
static INT32 mtk_wcn_soc_sw_init(P_WMT_HIF_CONF pWmtHifConf)
{
	...
	if (g_pdev!=NULL) {
		if( of_property_match_string(g_pdev->dev.of_node,"regulator-name","externalFEM")==0 ) {
			 WMT_ERR_FUNC("ePA: wifi_config\n");
			 ...
			 WMT_ERR_FUNC("iPA: Not set wifi_config\n");
		}
	} else{
		WMT_ERR_FUNC("g_dev or g_pdev->dev or g_pdev->dev.of_node are NULL !!! \n");
	}
